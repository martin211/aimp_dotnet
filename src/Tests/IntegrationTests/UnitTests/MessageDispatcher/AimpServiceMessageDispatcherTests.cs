...
public class AimpServiceMessageDispatcherTests : AimpIntegrationTest
{
    ...
    public void Hook_ShouldSetHook()
    {
        ExecuteInMainThread(() =>
        {
            ...
            AimpAssert.AreEqual(ActionResultType.OK, result.ResultType, "Cannot hook a new hook");
    ...
    public void Unhook_ShouldUnhook()
    {
        ExecuteInMainThread(() =>
        {
            ...
            AimpAssert.AreEqual(ActionResultType.OK, result.ResultType, "Cannot unhook a hook");
        });
    }
    ...
    public void Send_CheckIsCommandAllowed_ShouldBeOK(AimpCoreMessageType messageType, AimpCoreMessageType param1)
    {
        ExecuteInMainThread(() =>
        {
            ...
            var result = Player.ServiceMessageDispatcher.Send(messageType, (int)param1, ref pointer);
            AimpAssert.IsTrue(
                result.ResultType == ActionResultType.OK || result.ResultType == ActionResultType.Fail ||
                result.ResultType == ActionResultType.NotImplemented, $"Current value {result.ResultType}");
        });
    }
    ...
    public void Send_PropertyValueSet_ShouldBeCorrect(AimpCoreMessageType command, object param2)
    {
        ...
        {
            var validate = true;
            var outParam2 = IntPtr.Zero;
            var outType = AimpCoreMessageType.CmdAbout;
            ...
            var result = Player.ServiceMessageDispatcher.Hook(hook);
            AimpAssert.AreEqual(ActionResultType.OK, result.ResultType, "Cannot register a new hook");

            var pointer = param2.ToPointer();
            var cmdResult = Player.ServiceMessageDispatcher.Send(command, (int)AimpCoreMessageType.PropertyValueSet,
                ref pointer);
            AimpAssert.AreEqual(ActionResultType.OK, cmdResult.ResultType);
            AimpAssert.IsTrue(outParam2 != IntPtr.Zero);
            Player.ServiceMessageDispatcher.Unhook(hook);
            ...
            switch (outType)
            {
                ...
                case AimpCoreMessageType.PropertyShuffle:
                    AimpAssert.AreEqual(param2, outParam2.ToBool());
                    break;
                ...
                case AimpCoreMessageType.PropertyVolume:
                    AimpAssert.AreEqual(param2, outParam2.ToDouble());
                    break;
            }
        });
    }
    ...
    public void Send_HWND_ShouldReturnDescriptor(AimpDescriptorType windowId)
    {
        ExecuteInMainThreadAndWait(() =>
        {
            ...
            var hwnd = 0.ToPointer();
            var cmdResult =
                Player.ServiceMessageDispatcher.Send(AimpCoreMessageType.PropertyHwnd, (int)windowId, ref hwnd);
            AimpAssert.AreEqual(ActionResultType.OK, cmdResult.ResultType);
    ...
    public void Register_EmptyMessage_ShouldThrowException()
    {
        ExecuteInMainThread(() =>
        {
            AimpAssert.Throw<ArgumentNullException>(
                () => Player.ServiceMessageDispatcher.Register(string.Empty));
        });
    }
    ...
    public void Hook_Null_ShouldThrowException()
    {
        ExecuteInMainThread(() =>
        {
            AimpAssert.Throw<ArgumentNullException>(() => Player.ServiceMessageDispatcher.Hook(null));
        });
    }
}
